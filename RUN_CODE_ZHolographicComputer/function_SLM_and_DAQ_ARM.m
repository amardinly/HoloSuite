...
set(handles.statusText,'String','Holograms successfully loaded')
...
elseif  min(state-measure)>=0;
    ...
    elseif measure == [0,1]; 
         ...
         % set(handles.statusText,'String','NEXT SEQUENCE REQUESTED, waiting for pulse to load first hologram')
          if sequenceID == 1;
    ...
    if holoID>0
     ...
     %   set(handles.currentROI,'String',strcat('Now displaying -- ',int2str(holoID)))
       ...
       Screen('DrawTexture', w,ScreenFrames(TheSequence{sequenceID}(holoID))); %used to be this
       % way.  inexplicable random error occured, changed holoID to cell to
       % fix.  9/16/2016 11:30 am;
