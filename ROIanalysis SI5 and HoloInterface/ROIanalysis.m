...
avgImg=gd.Images.Computations.Data.Average;  %(:,:,:,1)
greenImg=avgImg(:,:,gd.state.currentDepth,1);
redImg=avgImg(:,:,gd.state.currentDepth,2);

for n = 1:numel(ROIdata)
    ROIdata.rois(n).greenImg = greenImg.*ROIdata.rois(n).mask;
    ROIdata.rois(n).redImg = redImg.*ROIdata.rois(n).mask;
end;
