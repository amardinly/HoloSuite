function [AverageFrame, MaxProjection, MinProjection, Variance, Kurtosis] = computeProjections(ImageFile, ExperimentFile, varargin)
...
if ~exist('ImageFile','var') || isempty(ImageFile) % Prompt for file selection
    ...
    [ImageFile,p] = uigetfile({'*.imgs;*.sbx;*.tif'},'Select images:',directory,'MultiSelect','on');
    if isnumeric(ImageFile)
        ImageFile = []; return
    elseif iscellstr(ImageFile)
        for index = 1:numel(ImageFile)
            ImageFile{index} = fullfile(p,ImageFile{index});
        end
    else
...
[Images, dataConfig] = load2P(ImageFile{1}, 'Type', 'Direct', 'Frames', 2, 'Double');
mem = memory;
...
fileConfig = load2PConfig(ImageFile);
...
if computeAverage || computeMax || computeMin
    ...
    Frames = load2P(ImageFile, 'Type', 'Direct', 'Double'); %memmap
