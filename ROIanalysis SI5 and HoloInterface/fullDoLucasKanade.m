...
if MCdataFilename == true
    ...
    [bindex,p] = uigetfile({'*.mat'}, 'Save alignment data to:', directory);
    if isnumeric(bindex)
        return
    end
    MCdataFilename = fullfile(p, bindex);
end
if MCImgsFilename == true
    directory = CanalSettings('DataDirectory');
    [bindex,p] = uiputfile({'*.sbx'}, 'Save registered images as:', directory);
    if isnumeric(bindex)
        return
    end
    MCImgsFilename = fullfile(p, bindex);
end
