...
% Now uses ScanImageTiffReader for Tiff files. it is very efficient but
% requires optimized compilers etc...
        ...
        for index = 1:numFiles
            ...
            if ~isempty(FrameIndex{index})
                switch loadObj.files(index).ext
                    ...
                    case '.tif'
                        %Images(col,row,chan,depth,frame)
                        %                         Images(:,:,:,:,startFrame(index):startFrame(index)+numFrames(index)-1)...
                        %                             = readScim(ImageFiles{index}, 'Frames', FrameIndex{index}, 'Channels', Channels, 'Depth', [1:Config(1).Depth]);
...
%% Load MemMap
    case 'MemMap'
        ...
        [~,~,loadObj.files.ext] = fileparts(ImageFiles);
        switch loadObj.files.ext
            case '.sbx'
                Images = readSbx(ImageFiles, [], 'Type', LoadType);
            case '.tif'
        ...
        loadObj.files.FramesLoaded = 1:Config.Frames;
end


%% Update data information
loadObj.Precision = class(Images);
...
loadObj.ZStepSize = Config.ZStepSize;
