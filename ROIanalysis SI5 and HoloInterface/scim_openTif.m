...
elseif strncmp('Frame',headerString,5);
    sIndex=strfind(headerString,'scanimage');
    if ~strncmp('scanimage.SI5.VERSION_MAJOR = 5',headerString(sIndex(1):sIndex(2)-1),31)
       disp( 'Error! expecting scanimage 5, stupid!');
    end
    %scanimage5
    fileVersion = 5;
    header = parseSI5Header(headerString);
else
    fileVersion = 4;
...
fiveD=0;
if numSlices > 1 && numFrames > 1
    selection = selectImages({'frames' 'slice'},numSlices*numFrames);
    selectionStr = 'Slice and Frame';
    numTotal = numSlices*numFrames;
    fiveD=1;
elseif numSlices > 1  
...
elseif fiveD==1   
    Aout = zeros(numLines,numPixels,length(selChans),numSlices,numFrames,imageDataType);
else
    Aout = zeros(numLines,numPixels,length(selChans),numSelections,imageDataType);
end
...
elseif fiveD == 1;
    ...
    try
        for i=1:(numSlices);
            for k = 1:(numFrames);
                for j = 1:length(savedChans)
                    idx = numChans * (selection(i*k) - 1) + j; %VI040511A
                   
                    if ismember(savedChans(j), selChans)
                        hTif.setDirectory(idx);
                        Aout(:,:,j,i,k) = hTif.read();
                    end
                end
            end
        end
...
%% GENERAL HELPERS
    ...
    function s = extractHeaderData(header,fileVersion)
        ...
        elseif fileVersion == 5
            ...
            s.acqLUT = cell(1,4);
            for i=1:length(s.acqLUT)
                eval(['s.acqLUT{i} = localHdr.chan' num2str(i) 'LUT;']);
            end
        else 
            assert(false);
        end
